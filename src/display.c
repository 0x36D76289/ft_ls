...
static void display_permissions_acl(t_file *file)
{
...
static void print_aligned_num(long num, int width)
{
    char *num_str;
...
static long calculate_blocks(t_file *files)
{
    t_file *current;
    long total;
...
{
    if (!(options->flags & OPT_COLOR) || (options->flags & OPT_F))
        return ("");
...
static void display_long_format_w(t_file *file, t_column_widths *widths, t_options *options)
{
    display_permissions_acl(file);
    ft_putchar_fd(' ', 1);

    print_aligned_num(file->stat.st_nlink, widths->nlink_width);
    ft_putchar_fd(' ', 1);
    ...
    print_aligned_num(file->stat.st_size, widths->size_width);
    ft_putchar_fd(' ', 1);

    if ((options->flags & OPT_U) && ((options->flags & OPT_T) || (options->flags & OPT_L)))
...
void display_long_format(t_file *file)
{
    ...
    display_long_format_w(file, &widths, &default_options);
}
...
void display_files(t_file *files, t_options *options)
{
    ...
    current = files;
    // -f disables long format, even if -l or -g is specified
    if ((options->flags & OPT_L || options->flags & OPT_G) && !(options->flags & OPT_F))
    {
        widths = calculate_column_widths(files);
        ft_putstr_fd("total ", 1);
        print_aligned_num(calculate_blocks(files), 0);
        ft_putchar_fd('\n', 1);

        while (current)
        {
            display_long_format_w(current, &widths, options);
            current = current->next;
        }
    }
    else
